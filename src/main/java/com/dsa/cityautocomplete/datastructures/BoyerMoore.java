...
public class BoyerMoore {
    ...
    public int search(String pattern, String text) {
        ...
        int charTable[] = makeBC_Table(patternArr);
        int offsetTable[] = makeGS_Table(patternArr);
        ...
        for (int i = patternArr.length - 1, j; i < textArr.length;) {
            ...
            i += Math.max(offsetTable[patternArr.length - 1 - j], charTable[textArr[i]]);
        }
        return -1;
    }

    private int[] makeBC_Table(char[] pattern) {
        final int ALPHABET_SIZE = 9000;
